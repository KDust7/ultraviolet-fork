...
class HTML extends EventEmitter {
	...
	async wrapSrcset(str, meta = this.ctx.meta) {
		const partsArr = await Promise.all(
			str.split(",").map(async (src) => {
				const parts = src.trimStart().split(" ");
				if (parts[0]) parts[0] = await this.ctx.rewriteUrl(parts[0], meta);
				return parts.join(" ");
			})
		);
		return partsArr.join(", ");
	}
	async unwrapSrcset(str, meta = this.ctx.meta) {
		const partsArr = await Promise.all(
			str.split(",").map(async (src) => {
				const parts = src.trimStart().split(" ");
				if (parts[0]) parts[0] = await this.ctx.sourceUrl(parts[0], meta);
				return parts.join(" ");
			})
		);
		return partsArr.join(", ");
